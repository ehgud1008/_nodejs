...
exports.authorList = (request, response) => {
  conn.query(`SELECT * FROM topic`, (err, topic, fields) => {
    conn.query(`SELECT * FROM author`, (err2, author, fields2) => {
      var title = "Author List";
      var description = "<a href='/authorCreate'>authorCreate</a>";
      var list = template.list(topic);
      var html = template.HTML(title,list,
        `<h1>${title}</h1>
        <h4>${description}</h4>
...
exports.authorCreate = (request,response) => {
  
}

exports.authorDelete = (request, response) => {
  var _url = request.url;
  var queryData = url.parse(_url, true).query;
  var body = "";
  request.on("data", function (data) {
  ...
  request.on("end", function () {
    ...
    conn.query(
      `DELETE FROM author WHERE id = ?`, [queryData.id], (error, result) => {
      if(error){
        throw error;
      }
      response.writeHead(302, { Location: `/authorList` });
      response.end();
    })
  });
}